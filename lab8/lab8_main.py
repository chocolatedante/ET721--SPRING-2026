...
#mean method calculates and returns the mean value of each row of the dataframe (df)
...
pit.bar([i - bar_width/2 for i in x], home_values, width=bar_width, label='Home', color='skyblue')
pit.bar([i - bar_width/2 for i in x], away_values, width=bar_width, label='Away', color='orange')

pit.xticks(x, metrics)
pit.title("GSW vs Raptors")

pit.ylabel("Average value")
pit.legend()
pit.show(block=True)
